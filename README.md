...
오전에는 누적 10건, 오후 발행을 포함한 하루 전체는 20건을 목표·상한으로 한다. 긱뉴스는 이 20건 중 하루 최대 10건이다. 소스별 발송 횟수를 한국 시간 기준으로 저장하므로 여러 예약 회차에 걸쳐서도 상한을 지킨다.
...
`GEMINI_API_KEY`가 없어도 선택된 영문 제목과 공개된 짧은 발췌를 무료 MyMemory API로 번역하고, 기사 내용에 맞춘 `왜 봐야 하나`를 붙인다. 텔레그램에는 **뉴스 1개당 게시물 1개**로 한국어 제목과 영문 원제를 함께 표시한다. 상단 한국어 제목 전체가 원문 링크이고 게시물 아래에도 큰 `원문 기사 바로 보기` 버튼을 붙인다. 키를 넣으면 기사 묶음 전체를 한 번 호출해 한국어 제목·요약·실무 포인트를 만든다.

한국어 제목·요약이 완성된 글만 발행한다. 번역 실패·영문 반환·누락된 요약은 영어 원문으로 대체하지 않고 해당 글을 이번 회차에서 제외한다. 읽음 기록과 일일 발송 횟수는 올리지 않으므로 다음 실행에서 다시 번역할 수 있다. 빈자리는 다음 후보로 보충하며, 긱뉴스 등 한국어 기사 발행은 계속한다. 한 실행 중 번역 제공자가 실패하면 같은 제공자를 후보마다 반복 호출하지 않고 다음 실행에서 재시도한다. 번역 서비스가 복구되거나 Gemini 키가 설정되기 전에는 영문 소스의 발행량이 줄어들 수 있다.
...
[`.github/workflows/digest.yml`](.github/workflows/digest.yml)은 **한국 시간 낮 12시 전 뉴스 최소 10건**을 목표로 오전 발행을 집중한다.

| 한국 시간 | 누적 발행 목표 / 실행 방식 |
| --- | --- |
| 07:13 · 07:43 · 08:13 | 오늘 누적 3건까지 부족분 발행 |
| 08:43 · 09:13 · 09:43 | 오늘 누적 6건까지 부족분 발행 |
| 10:13 · 10:43 · 11:13 · 11:43 | 오늘 누적 10건까지 부족분 발행 |
| 12:53 · 14:23 · 15:53 · 17:23 · 18:53 · 20:23 · 21:53 | 기존 90분 간격, 회차당 최대 2건으로 하루 총 20건까지 발행 |

앞 회차가 실패하거나 기사를 적게 보냈으면 다음 실행이 **실제 성공 누적량**을 읽어 부족분을 보충한다. 예를 들어 10시대에 오늘 2건을 보냈다면 최대 8건을 선택한다. 10건을 채운 오전 회차에는 반응만 수집하고 뉴스 수집·발송은 생략한다. 원문 확인 중 중복 기사가 탈락하면 차순위 후보를 채우며, 긱뉴스 하루 10건과 회차별 출처 상한을 계속 지킨다.

오후에도 새 뉴스를 계속 보내며 하루 전체는 20건을 목표·상한으로 한다. 오전 10건을 채운 뒤 오후 다섯 회차에 각 2건씩 보내면 20건이 되고, 남은 두 회차는 부족분 재시도에 쓴다. 정오에 10건 미달이면 오후 회차에도 부족분 보충을 시도하되, 이는 정오 전 목표 달성으로 간주하지 않는다. 예약 실행이 지연되어도 실제 KST 시간을 기준으로 발행량을 계산하고, 07시 전·22시 이후에는 자동 발행하지 않는다. 단건 지정·명시적 `--limit`·소스 지정·레이아웃 미리보기는 자동 증량 대상에서 제외한다.

**10건은 운영 목표이며 기사 수와 실행 시각을 보장하는 수치는 아니다.** GitHub 예약이 지연되거나 적합한 새 기사가 부족하면 미달할 수 있다. 후보 수·최종 선정 수·부족량은 실행 로그에 남기고 다음 회차에 다시 시도한다. 수량을 채우려고 읽은 기사나 오래된 글, 무관한 뉴스를 발행하지 않는다.
