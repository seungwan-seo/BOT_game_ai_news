...
오전에는 누적 10건을 목표로 하고, 오후 발행을 포함한 하루 전체 상한은 20건이다. 긱뉴스는 계속 하루 최대 2건이다. 소스별 발송 횟수를 한국 시간 기준으로 저장하므로 여러 예약 회차에 걸쳐서도 상한을 지킨다.
...
| 12:53 · 14:23 · 15:53 · 17:23 · 18:53 · 20:23 · 21:53 | 기존 90분 간격, 회차당 새 기사 1건 |

앞 회차가 실패하거나 기사를 적게 보냈으면 다음 실행이 **실제 성공 누적량**을 읽어 부족분을 보충한다. 예를 들어 10시대에 오늘 2건을 보냈다면 최대 8건을 선택한다. 10건을 채운 오전 회차에는 반응만 수집하고 뉴스 수집·발송은 생략한다. 원문 확인 중 중복 기사가 탈락하면 차순위 후보를 채우며, 긱뉴스 하루 2건과 회차별 출처 상한을 계속 지킨다.

오후에도 새 뉴스를 계속 보내며 하루 전체는 최대 20건이다. 정오에 10건 미달이면 오후 회차에도 부족분 보충을 시도하되, 이는 정오 전 목표 달성으로 간주하지 않는다. 예약 실행이 지연되어도 실제 KST 시간을 기준으로 발행량을 계산하고, 07시 전·22시 이후에는 자동 발행하지 않는다. 단건 지정·명시적 `--limit`·소스 지정·레이아웃 미리보기는 자동 증량 대상에서 제외한다.
